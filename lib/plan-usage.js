...
/**
 * Rolling 7-day session generation counter. Used for freemium weekly quota.
 * @param {import('pg').Pool} pool
 * @param {string} userId
 * @returns {Promise<number>}
 */
async function getAiGenerationsLast7Days(pool, userId) {
	if (!pool || !userId) return 0;
	const { rows } = await pool.query(
		`SELECT COALESCE(SUM(ai_generations), 0)::int AS n
		 FROM public.plan_usage_daily
		 WHERE user_id = $1::uuid
		   AND usage_date > (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date - INTERVAL '7 days'`,
		[userId]
	);
	return rows[0]?.n ?? 0;
}

/** ISO timestamp 7 days from now — rough "quota refreshes by" hint for UI. */
