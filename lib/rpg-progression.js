 ...
 */
/**
 * XP for completing a solo generated activity after the timer + reflection.
 * Scales with the configured upper duration (minutes) — longer = harder.
 * Pure function — used only server-side in /api/profile/complete-solo.
 *
 * @param {number} durationMaxMinutes — game_json.duration.max (timer uses same max)
 * @returns {number} integer XP in [30, 120]
 */
function computeSoloXpFromDurationMax(durationMaxMinutes) {
  const mRaw = parseInt(durationMaxMinutes, 10);
  const m = Number.isFinite(mRaw) && mRaw > 0
    ? Math.max(5, Math.min(180, mRaw))
    : 15;
  const scaled = Math.round(25 + (m - 5) * 2.8);
